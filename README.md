...
For this, I figured we could use floats so that we could return more accurate averages. And, I created a loop of functions that:
  ...
  1. get user input (a switch statement that checks the *first* character in the user input to see what function to call)
  2. process it (because I use getline() to get the input as a string and then convert it to a float if possible)
  ```
  ...
  ```
  3. store it in a vector so the size can change
  ```
  ...
  ```
  4. and then wait for more user input. 
